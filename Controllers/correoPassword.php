<?php   
    ...
    if (!isset($_SESSION['token'])) {
        if(!isset($_SESSION["info"])){
            $correo = $_POST['correo'];
            if(!$objUsu->usuarioExiste($correo)){
                $_SESSION["flague"] = true;
                header("Location: ../Views/Informacion/cambiarPass.php");
                die();
            }
        }else{
            $correo = $_SESSION['info'][0]["Correo"];
        }
        if ($_POST["pass"] != $_POST["passconf"]) {
            $_SESSION["flagdifpass"] = true;
            header("Location: ../Views/Informacion/cambiarPass.php");
            die();
        }   
...
<body>
    <div role="article" aria-roledescription="email" lang="en" class="contenedor">
                    ...
                    <table role="presentation" class="tabla-interna">
                        ...
                        <tr>
                            <td class="parte-dos">
                                ...
                                <p>Se solicitó un código de verificación para cambiar su contraseña. Copie su código:</p>
                            </td>
                        </tr>
                        <tr>
                            <td class="parte-tres">
        ...
        if(mail($_SESSION['correo'], 'Codigo para cambio de contraseña', $mensaje,$cabeceras)){
            header("Location: ../Views/Informacion/validarCorreoPass.php");
            die();
        }else{
            echo "error";
        }
